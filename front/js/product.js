  ...
  .then((selectedProduct) => {
    ...
    addToCartButton.addEventListener("click", (Event) => {
      ...
      if (
        ...
        quantityChoice == undefined ||
        quantityChoice == null ||
        quantityChoice < 1 ||
        quantityChoice > 100
      ) {
        alert(
          "Veuillez sélectionner une couleur et une quantité valide (entre 1 et 100) pour ce produit"
        );
      }
      // Sinon, nous ajoutons le produit et ses caractéristiques au panier.
        ...
        const addProductToLocalStorage = () => {
          ...
          let found = basket.find((element) => {
            return (
              element.idProduct === productOption.idProduct &&
              element.colorProduct === productOption.colorProduct
            );
          });
          console.log(found);
          ...
          if (found) {
            ...
            if (total > 100) {
              alert(
                "La quantité totale d'un produit ne doit pas être supérieure à 100. Merci d'effectuer une autre commande en complément de celle-ci si votre demande est supérieure à 100 unités d'un même modèle."
              );
            }
            // Sinon, on définit la quantité sélectionnée comme celle du panier.
              ...
              alert(
                `Le ${selectedProduct.name} de couleur ${colorChoice} a bien été ajouter à votre panier.`
              );
          ...
          // Si le produit n'est pas présent, on informe l'utilisateur et on ajoute les caractéristiques choisies au Live Server.
            ...
            alert(
              `Vous avez bien ajouté ${quantityChoice} ${selectedProduct.name} de couleur ${colorChoice} à votre panier.`
            );
          }
